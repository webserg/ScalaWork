object subset {
  ...
  def isOk(l: List[(Char, Int)]): Boolean = l match {
    case Nil => true
    case head :: tail => if (tail.length > 1) false else if (tail.isEmpty) {
      true
    } else {
      if (tail.head._1 == head._1) false else true
    }
  } //> isOk: (l: List[(Char, Int)])Boolean

  ls.map {
    _.toList
  } filter (isOk(_)) //> res0: List[List[(Char, Int)]] = List(List(), List((b,1)), List((a,2), (b,2))
